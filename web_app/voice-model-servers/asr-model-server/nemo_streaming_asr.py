...
from nemo_streaming_utils import CacheFeatureBufferer
...
class NemoStreamingASRService:
    ...
    def transcribe_core(self, audio: np.ndarray, start_time: float, stream_id: str = "default") -> ASRResult:

        self._audio_buffer.update(audio)
    ...
    def transcribe(self, audio: bytes, stream_id: str = "default") -> ASRResult:
        ...
        audio_array = np.frombuffer(audio, dtype=np.int16).astype(np.float32) / 32768.0     # Convert bytes to numpy array
        return self.transcribe_core(audio_array, start_time, stream_id)

    def transcribe_float32(self, audio: np.ndarray, stream_id: str = "default") -> ASRResult:
        start_time = time.time()
        return self.transcribe_core(audio, start_time, stream_id)
