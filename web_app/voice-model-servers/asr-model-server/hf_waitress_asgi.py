...
def shutdown_all():
    ...
    if SILERO_VAD:
        try:
            print("Attempting to free Silero VAD")
            del SILERO_VAD
            print("Silero VAD freed successfully")
        except Exception as e:
            handle_error_no_return("Could not free Silero VAD, encountered error: ", e)
        finally:
            SILERO_VAD = None

    if SILERO_UTILS:
        try:
            print("Attempting to free Silero VAD utils")
            del SILERO_UTILS
            print("Silero VAD utils freed successfully")
        except Exception as e:
            handle_error_no_return("Could not free Silero VAD utils, encountered error: ", e)
        finally:
            SILERO_UTILS = None

    print("\n\nSilero VAD cleanup complete\n\n")
...
def load_vad_pipeline():
    ...
    try:
        vad_config = read_config(['asr_vad_model', 'asr_vad_device'])
    except Exception as e:
        handle_local_error("Could not read values from hf_config.json when attempting to load vad-pipeline, encountered error: ", e)

    try:
        if vad_config['asr_vad_model'].lower().strip() == 'snakers4/silero-vad':
            SILERO_VAD, SILERO_UTILS = torch.hub.load('snakers4/silero-vad', 'silero_vad', trust_repo=True)
            SILERO_VAD.to(vad_config['asr_vad_device']).eval()
        else:
            raise ValueError(f"Invalid VAD model ID: {vad_config['asr_vad_model']}")
    except Exception as e:
...
class ASRWebSocket(WebSocketEndpoint):
    ...
    async def on_connect(self, websocket):
        ...
        def set_if(name, cast=float):
            if name in qp:
                try: self.asr_cfg[name] = cast(qp[name])
                except: pass
        ...
        self.audio_to_process = self.audio_buffer.copy()
        self.last_speech_time = time.time()
        self.global_cursor = 0   # absolute sample index of audio_buffer[0]
        self.last_append_end_abs = 0   # absolute sample index up to which we've appended into audio_to_process


    async def _transcribe_async(self, audio_f32: np.ndarray, asr_config: dict) -> str:
        loop = asyncio.get_running_loop()
        def do_asr():
            transcription = transcribe_audio_data_with_asr_model(audio_f32, asr_config)
            return transcription if transcription else ""
        return await loop.run_in_executor(None, do_asr) # Keep heavy ASR inference off the event loop: Use await loop.run_in_executor(None, do_asr) for blocking ASR-run calls.
        ...
        if len(self.audio_buffer) >= self.asr_cfg['asr_min_meaningful_samples_factor'] * self.asr_cfg['asr_samplerate']:
            with torch.no_grad():
                ...
                segments = self.get_speech_timestamps(
                    wav_t,
                    self.silero_vad,
                    sampling_rate=self.asr_cfg['asr_samplerate'],
                    threshold=self.asr_cfg['asr_vad_threshold'],
                    min_speech_duration_ms=self.asr_cfg['asr_vad_min_speech_ms'],
                    min_silence_duration_ms=self.asr_cfg['asr_vad_min_silence_ms'],
            ...
            for segment in segments:
                ...
                if append_start_abs < abs_end:
                    ...
                    self.audio_to_process = new_chunk if len(self.audio_to_process) == 0 else np.concatenate((self.audio_to_process, new_chunk))
            ...
            if self.audio_to_process.size >= self.asr_cfg['asr_min_chunk_duration_s'] * self.asr_cfg['asr_samplerate'] and (time.time() - self.last_speech_time) > self.asr_cfg['asr_silence_duration_s']:
                ...
                if self.asr_cfg['asr_apply_normalization']:
                    print(f"Applying normalization to audio.")
                    peak_volume = np.max(np.abs(self.audio_to_process)) if len(self.audio_to_process) > 0 else 0.0
                    if peak_volume > 0: # normalize the spoken audio - also helps overcome the audio-level differences between the spoken audio and the padding audio!
                        self.audio_to_process = self.audio_to_process / peak_volume
                    ...
                    if len(self.audio_to_process) < self.asr_cfg['asr_min_context_s'] * self.asr_cfg['asr_samplerate']:
                        ...
                        if self.asr_cfg['asr_apply_rms_dimming']:
                            ...
                            if self.asr_cfg['asr_apply_crossfade']:
                                print(f"Applying crossfade to padding audio.")
                                # short crossfade to avoid a hard boundary
                                xf = int(0.05 * self.asr_cfg['asr_samplerate'])  # 50 ms
                                if len(speech) >= xf and len(padding_audio_dimmed_rms) >= xf:
                                    fade = np.linspace(1.0, 0.0, xf, dtype=np.float32)
                            ...
                            print(f"Applying padding audio without RMS dimming.")
                            self.audio_to_process = np.concatenate((self.audio_to_process, self.padding_audio))
                            padding_applied = True

                if self.asr_cfg['asr_apply_zero_padding'] and not padding_applied:
                    print(f"Applying zero-padding to audio.")
                    needed = max(0, int(self.asr_cfg['asr_min_context_s'] * self.asr_cfg['asr_samplerate'] - len(self.audio_to_process)))
                    if needed > 0:
                        print(f"Audio is shorter than {self.asr_cfg['asr_min_context_s']}s. Applying zero-padding.")
                        self.audio_to_process = np.concatenate((self.audio_to_process, np.zeros(needed, dtype=np.float32)))

                transcription = await self._transcribe_async(self.audio_to_process, self.asr_cfg)
...
def build_asgi_app():
    mounted_flask = WsgiToAsgi(app)
    starlette_app = Starlette(routes=[
        WebSocketRoute("/ws/asr", ASRWebSocket),    # Mount ASGI routes first otherwise '/' will grab everything!
        WebSocketRoute("/ws/asr/", ASRWebSocket),  # optional: match trailing slash too
        Mount("/", app=mounted_flask),
    ])
    return starlette_app
    '''
    WebSockets can be done in two ways:

    1. Class based WebSocket endpoints `class AsrWebSocket(WebSocketEndpoint)`
        - Route with WebSocketRoute("/ws/asr", AsrWebSocket).
        - Methods include on_connect, on_receive, on_disconnect, etc. These are abstract methods provided by the WebSocketEndpoint class and must be implemented by the user.
        - The WebSocketEndpoint class can be found in the endpoints.py file (starlette/endpoints.py) in the starlette package
        - Inspect the WebSocketEndpoint class when importing: `from starlette.endpoints import WebSocketEndpoint`
    
    2. Function based WebSocket endpoints `async def asr_websocket(websocket: WebSocket)`
        - Manually await websocket.receive() inside a loop.
        - Route with WebSocketRoute("/ws/asr", endpoint=ws_asr).
        - Other methods include await websocket.accept(), await websocket.send_text("Hello, world!"), await websocket.close(), etc. These methods are provided by the WebSocket class.
        - The WebSocket class can be found in the websockets.py file (starlette/websockets.py) in the starlette package, inspect via: `from starlette.websockets import WebSocket`
        - Or, inspect WebSocketRoute above, which will open routing.py and scroll up to see the import mentioned above and inspect the WebSocket class there.

    Class based WebSocket endpoints are more flexible and can be used to add more complex logic to the WebSocket connection.
    Function based WebSocket endpoints are simpler and can be used to add basic logic to the WebSocket connection.
    '''
...
## Wrapping Flask with WsgiToAsgi and adding a WebSocket route is a well-trodden, recommended bridge—not a hack unless you decide to go full ASGI later.

## Why this exists:
    - WSGI is sync HTTP-only. No native WebSocket support, limited concurrency patterns.
    - ASGI is async and supports HTTP, WebSockets, SSE, background tasks, and backpressure.
    - Wrapping lets you preserve your current Flask app (all GET/POST endpoints, config/shutdown APIs), while adding ASGI-native features like WebSockets in the same process.

## Motivations:
    - Maintain all your current behavior and single-file ergonomics.
    - Add a bidirectional streaming ASR route without rewriting your Flask app.
    - Avoid running a second process/port (if you prefer a single service). You can still keep a sidecar if you want strict separation.

## Alternatives (when to choose them):
    - Separate ASGI sidecar (recommended if you want zero risk): Keep Waitress on 9069; run a tiny FastAPI/Starlette app on 9070 with /ws/asr. Pros: total isolation. Cons: two ports/processes.
    - Quart + Hypercorn: Quart is a Flask-compatible ASGI framework with native WebSockets. Pros: minimal migration (swap imports), one server. Cons: some Flask extensions behave differently; mild refactor.
    - Flask-SocketIO (eventlet/gevent): Socket.IO protocol (not raw WS), monkey-patching; can conflict with heavy CPU/GPU workloads and is a different ecosystem.
    - Full FastAPI rewrite: best long-term for highly async apps; more effort now.

## When to wrap vs sidecar
    - Wrap (single executable/port): You want “copyable single file” and your GPU node runs one worker. Keep concurrency inside the process with asyncio/threadpools.
    - Sidecar (two ports): You want strict separation, can scale WS independently, or keep Waitress unchanged in prod.

## Concurrency and performance tips
    - Keep heavy ASR inference off the event loop: Use await loop.run_in_executor(None, do_asr) for blocking Whisper calls.
    - GPU sharing: Prefer a single process/worker to avoid multiple processes contending for the same GPU. Use semaphores (you already do) for safe serialization where needed.
    - Backpressure: If you support multiple clients, maintain a small, bounded queue per WebSocket to drop late frames under load.

## CORS, origins, and proxies
    - Your existing CORS(app) covers HTTP calls. CORS doesn't apply to WebSockets the same way—handle origin checks in your WS on_connect or enforce via reverse proxy.
    - Behind Nginx/Caddy, ensure WS upgrade headers are passed through. Use WSS in production.

## Lifecycle and shutdown:
    - Your existing shutdown API remains (wrapped). If you use os._exit(0), it still terminates the process. Optionally add ASGI lifespan hooks to clean up models and CUDA.

## Debugging and testing:
    - Browser DevTools: verify binary WS frames up and JSON transcript messages down.
    - CLI: websocat ws://host:port/ws/asr (send raw PCM16 frames for quick checks).
    - Health endpoints: keep your /health route; you can add a Starlette one too.

## Future evolution:
    - Move to WebRTC for full “VoIP-like” behavior (NAT traversal, jitter buffering, AEC). Start with WS for simplicity, then upgrade.
    - Add interim (partial) transcripts if you implement chunk-wise decoding; start with final-after-silence for stability.

## Common pitfalls
    - Don't block the event loop with synchronous GPU calls; always offload.
    - If you run multiple Uvicorn workers with a single GPU, you'll likely thrash VRAM. Keep --workers 1 and use async/task concurrency instead.
    - Starlette CORS middleware does not govern WebSocket origins; enforce origin yourself or rely on proxy rules.

## Minimal client impact
    - Keep your current record-to-file flow unchanged. Add a second “Stream” toggle that:
        - Captures mic frames every ~20ms.
        - Sends PCM16 frames via WebSocket.
        - Appends transcripts to userInput.value as JSON messages arrive.

## Production knobs
    - Uvicorn: --http h11 --ws websockets --loop uvloop --workers 1.
    - Reverse proxy timeouts for long-lived WS connections.
    - Auth tokens on WS, rate limits, and session duration caps.

Is the WSGI→ASGI wrap “hacky”? No. It's a standard, supported pattern. asgiref.wsgi.WsgiToAsgi is designed for exactly this: 
run a sync WSGI app (Flask) inside an ASGI server (Uvicorn/Hypercorn), while also exposing native ASGI endpoints (WebSockets).
Many teams use it as a pragmatic bridge.

## Key guidance and caveats (why/when/how)
- Why this is fine:
