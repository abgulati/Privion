...
docker_only_hf_config_path = '/app/lars_storage/waitress_storage/hf_config.json'    # we already know that a docker env is linux-debian, no need to wait for platform detection logic further below!
docker_directory = os.path.dirname(docker_only_hf_config_path)
...
if not os.path.exists(docker_only_hf_config_path):
    try:
        with open(docker_only_hf_config_path, 'w') as file:
            json.dump({}, file)
    except Exception as e:
...
def write_config(config_updates, filename=docker_only_hf_config_path):
...
def read_config(keys, default_value=None, filename=docker_only_hf_config_path):
    ...
    with reader_semaphore:
        ...
        for key in keys:
            if key in hf_config:
                ...
                default_value = {
                    'windows_base_directory':'C:/lars_storage/waitress_storage',
                    'unix_and_docker_base_directory':'/app/lars_storage/waitress_storage',
